...
import { parseCartaCSVWithDiagnostics } from '@/lib/csvParser';
import { fetchCartaCSVText } from '@/lib/menuDataFetcher';
...
export async function GET() {
  ...
  const fetched = await fetchCartaCSVText();
  const timestamp = new Date().toISOString();

  const rawCsvUrl = process.env.NEXT_PUBLIC_GOOGLE_SHEET_CARTA_CSV_URL;
  ...
  const diagnostics = parseCartaCSVWithDiagnostics(fetched.csvText);
